...
A transparent software supply chain offers benefits for vulnerabilities remediation, but also for licensing management and it provides assurance of the source and integrity of components. In order to use efficiently the SBOM and widespread its adoption, information must be generated and shared in a standard format. This format must be a machine-readable format to allow automation. NTIA white paper [“Survey of existing SBOM formats and standards”](https://www.ntia.doc.gov/files/ntia/publications/ntia_sbom_formats_and_standards_whitepaper_-_version_20191025.pdf) identifies two key formats covering baseline SBOM information: Software Package Data eXchange (SPX) and Software Identification Tags (SWID tags).
