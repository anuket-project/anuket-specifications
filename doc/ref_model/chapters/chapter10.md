...
The ability to dynaically scale a network function by load balancing accross multiple instances/replicas of the same VNF or CNF is essential. New architectures and application patterns such as micro services is making this even more crucial. It must not only be possible to scale each service layer independently, support to chain the different layers together with support from "Service Funtion Chaining" is also needed. 

The load balancing and scaling need for typical enterprise applications is well supported in Openstack by the LBaaSv2 API and Octavia, the built in mechanism in Kubernetes for scaling enterprise type of services and PODs, that use a single interface, is also sufficient.

WHat is not supported in neither Openstack nor Kubernetes is to scale and load balance3 typical VNF and CNF. There is no support in Openstack to scale an stateful L3 application, SCTCP, QUIC. mTCP, gRPC and so on. In Kubernetes it is even worse, there is no support at all to define how to define any sort of network functions for the secondary  service/POD interfaces and how the corresponding NWs are intrerconnected.
