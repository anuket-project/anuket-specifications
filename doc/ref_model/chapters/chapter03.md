...
<a name="3.6"></a>
## 3.6 Storage
The general function of storage subsystem is to provide the persistent data store for virtual resources and to the virtual and physical resources required for the delivery of a network service. Storage can be classified based on its: cost/performance, consumption model (platform native, network shared, object or archival) and capacity. In cloud infrastructure such storage may manifest itself in various ways like storage endpoints being exposed over network from software defined storage dedicated clusters or hyperconverged nodes (combining storage and other functions like compute or networking).
