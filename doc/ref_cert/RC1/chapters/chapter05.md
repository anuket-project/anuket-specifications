...
The NFV Infrastructure (NFVI) is the totality of all hardware and software components
 ...
 the Anuket CNTT is to define an agnostic NFVI and removes any dependencies between
...
**Defined**.  _Badging_  refers to the granting of a Conformance badge by the OVP to Suppliers/Testers of Anuket CNTT NFVI+VNF upon demonstration the testing performed confirms:
...
* *NFVi profiles*: List of supporting OVP Certified Anuket CNTT compliant NFVi
...
under Anuket CNTT. Platform should support to managed and execute these test cases.
...
configuration, update, termination) using MANO supporting Anuket CNTT compliant NFVI.

##### Validation

Perform various VNF type specific functionality operations on Anuket CNTT RA & RM
compliant NFVI
...
Platform would support to execute various test cases on Anuket CNTT RA & RM compliant
...
Anuket CNTT is defining the required architecture and model for NFVI which will help to decouple the various commercial product layers and it is important to define and certify the VNF and NFVI.Therefore,in addition to verify general NFVI capabilities based on Anuket CNTT RM/RA/RI, it is also necessary to verify that VNFs can provide virtualization functions normally based on the Anuket CNTT-compatible NFVI. So the VNF testing should at least include: Compliance，verification，validation，Performance. With the improvement of specifications, the types of tests may continue to add in the future.
...
- NFVI is ready and it should be Anuket CNTT-compatible NFVI
...
VNF test cases are used to verify whether the virtualization network functions can be deployed on the Anuket CNTT-compatible NFVI and provide normal functions and meet performance, security and other requirements.

By running these VNF test cases and analysis the test results, can be used for VNF compliance, verfication,validation and performance Conformance and help on Anuket CNTT-compatible NFVI validation and performance Conformance.
...
Anuket CNTT defines the following four category testing which should be consistent with the VNF test category defined by OVP.
...
Above compliance test cases defination can be found https://github.com/onap/vnfsdk-validation/tree/master/csarvalidation/src/main/resources/open-cli-schema
 ...
 In order to adapt Anuket CNTT specification, more compliance test case will be added here.
