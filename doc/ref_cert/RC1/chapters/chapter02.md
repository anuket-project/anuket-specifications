...
The normalized result of the compute performance test requires all the vCPUs to run parallel workloads. You need to start as many vCPUs as needed to force all the physical CPUs on the node to run a workload. In this case, the result is normalized:
