...
Kubernetes clusters using above enhancements can implement worker nodes with "bare metal" servers (running Container Runtime in Linux host Operating System directly on the hardware without any hypervisor) or with virtual machines (VMs, on hypervisor).
