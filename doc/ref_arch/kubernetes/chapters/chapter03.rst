...
The Memory Manager :cite:p:`k8s-docs-memorymanager`
...
An example of often used device plugin is the SR-IOV Network Device Plugin :cite:p:`k8s-sriov-network-device-plugin`, that discovers and advertises SR-IOV Virtual
Functions (VFs) available on a Kubernetes node, and is used to map VFs to scheduled pods. To use it, an SR-IOV CNI is
required. A CNI multiplexer plugin (such as Multus CNI :cite:p:`k8s-multus`) is
...
Plugins :cite:p:`k8s-docs-network-plugins`,
...
approaches that can be taken, with the table below showing some of the
...
Standard :cite:p:`k8s-multi-net-spec`.

- The **Default CNI Plugin** through the use of deployment specific configuration (e.g. Tungsten Fabric :cite:p:`tungstenfabric`)
...
For hardware resources that are needed by Kubernetes applications, Device Plugins :cite:p:`k8s-docs-deviceplugin` can
be used to manage those resources and advertise them to the kubelet for use by the Kubernetes applications. This allows
resources such as "GPUs, high-performance NICs, FPGAs, InfiniBand adapters, and other similar computing resources that
may require vendor specific initialisation and setup" to be managed and consumed via standard interfaces.
...
Network Special Interest Group :cite:p:`sig-network` and it's sub
groups. One such group, the Network Plumbing Working Group :cite:p:`k8s-npwg` has
produced the
...
plugins are moved out from the Kubernetes repository).
...
terms of the requirements that are defined in RM sections `storage configurations`
(hardware) and `virtual storage` (software). The only point of difference is that
...
component installed to the Kubernetes Cluster (e.g., Tiller), Helm v3 :cite:p:`helm-docs` is the chosen
Kubernetes Application package manager.
...
-  API Server Aggregation :cite:p:`k8s-docs-apisa`: Additional
...
Operator pattern :cite:p:`k8s-docs-operator` combines custom resources and
...
Operators can have different capability levels. As per repository OperatorHub.io :cite:p:`operatorhub`, an
